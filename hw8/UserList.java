...
public class UserList {
    ...
    **/ 
    public static void partition(String sortFeature, CatenableQueue<User> qUnsorted, int pivot, 
        ...
        while (qUnsorted.size() != 0) {
            User returned = qUnsorted.dequeue();
            if (sortFeature.equals("id")) { 
                if (returned.getId() < pivot ) {
                    //append to less queue
                    qLess.enqueue(returned);
                }
                else if(returned.getId() == pivot ) {
                    //append to equal queue
                    qEqual.enqueue(returned);
                }

                else if (returned.getId() > pivot) {
                    // append to more queue
                    qGreater.enqueue(returned);
                }
            }


            else if (sortFeature.equals("pages")) {
                if (returned.getPagesPrinted() < pivot) {
                    //append to less queue
                    qLess.enqueue(returned);
                }
                else if (returned.getPagesPrinted() == pivot) {
                    //append to equal queue
                    qEqual.enqueue(returned);
                }
                else if (returned.getPagesPrinted() > pivot) {
    ...
    **/
    public static void quickSort(String sortFeature, CatenableQueue<User> q){ 
        if (q.isEmpty()) {
            ...
            // int pivot = (int) Math.random() * q.size() - 1e-9;
            int pivot = (int) q.size() / 2;
            User pivoter = q.nth(pivot);

            if (sortFeature.equals("pages")) {
                pivot = pivoter.getPagesPrinted();
            }
            if (sortFeature.equals("id")) {
                pivot = pivoter.getId();
            }
            ...
            /* Okay, so now less, equal and more are filled with their respective elements
            /* Now we need to sort each of them individually, recursively */
            ...
            if (sortFeature.equals("id")) {
                    ...
                    quickSort("pages", less);
                    quickSort("pages", more);
            }
    ...
    **/
    public CatenableQueue<CatenableQueue<User>> makeQueueOfQueues(){
        ...
        /* Now, dequeue from userQueue, then enqueue to a new queue every time, then we put that new queue in the 
         * Queue of queues of users */
        while (userQueue.size() != 0) {
            User dequeued = userQueue.dequeue();
            CatenableQueue<User> putter = new CatenableQueue();
            putter.enqueue(dequeued);
            toReturn.enqueue(putter);
        }
        return toReturn;
    }
    ...
    **/
    public static CatenableQueue<User> mergeTwoQueues(String sortFeature, CatenableQueue<User> q1, CatenableQueue<User> q2){
        CatenableQueue<User> mergedQueues = new CatenableQueue<User>();
        if (q1.size() == 0) {
            while (q2.size() != 0) {
                mergedQueues.enqueue(q2.dequeue());
            }
        }
        else if (q2.size() == 0) {
            while (q1.size() != 0) {
                mergedQueues.enqueue(q1.dequeue());
            }
        } else {
            if (sortFeature.equals("id")) {
                User q1User = q1.front();
                    User q2User = q2.front();
                while (q1.size() != 0 && q2.size() != 0) {
                    if (q1User.getId() == q2User.getId()) {
                        mergedQueues.enqueue(q1.dequeue());
                        mergedQueues.enqueue(q2.dequeue());
                    } else {
                        while (q1User.getId() < q2User.getId() && q1.size() != 0) { 
                            q1User = q1.dequeue();
                            mergedQueues.enqueue(q1User);
                        }
                        // mergedQueues.enqueue(q2User);
                        if (q1.size() == 0) { // so if the other queue is just empty, we just append everything to it
                            while (q2.size() != 0) {
                                q2User = q2.dequeue();
                                mergedQueues.enqueue(q2User);           
                            }
                        } 
                        // now we need to check if the other one isnt empty, then we initiate another while loop.
                        while (q1User.getId() > q2User.getId() && q2.size() != 0) {
                            q2User = q2.dequeue();
                            mergedQueues.enqueue(q2User);
                        }
                    }
                ...
                User q1UserP = q1.front();
                User q2UserP = q2.front();
                while (q1.size() != 0 && q2.size() != 0) {
                    if (q1UserP.getPagesPrinted() == q2UserP.getPagesPrinted()) {
                        mergedQueues.enqueue(q1.dequeue());
                        mergedQueues.enqueue(q2.dequeue());
                    } else {
                        while (q1UserP.getPagesPrinted() < q2UserP.getPagesPrinted() && q1.size() != 0) { 
                            q1UserP = q1.dequeue();
                            mergedQueues.enqueue(q1UserP);
                        }
                        // mergedQueues.enqueue(q2UserP);
                        if (q1.size() == 0) { // so if the other queue is just empty, we just append everything to it
                            while (q2.size() != 0) {
                                q2UserP = q2.dequeue();
                                mergedQueues.enqueue(q2UserP);
                        ...
                        // now we need to check if the other one isnt empty, then we initiate another while loop.
                        while (q1UserP.getPagesPrinted() > q2UserP.getPagesPrinted() && q2.size() != 0) {
                            q2UserP = q2.dequeue();
                            mergedQueues.enqueue(q2UserP);   
                        }
                    }
